...
# 静默配置443端口信息 修改位置 Install_SSR(){  Set_config_all
# 已删  显示 菜单状态menu_status(){
# 已删  Update_Shell(){
# 已删  设置 配置信息Set_config_port()  至  {Set_config_all(){
# 已删  修改 配置信息
#=================================================
...
Install_SSR(){
	check_root
	[[ -e ${config_user_file} ]] && echo -e "${Error}ssr配置文件存在，可能已静默安装ssr，更新安装管理 wget -N --no-check-certificate -O ssr.sh https://git.io/JojzD && chmod +x ssr.sh " && exit 1
	[[ -e ${ssr_folder} ]] && echo -e "${Error} ssr文件夹存在，可能已静默安装ssr，更新安装管理 wget -N --no-check-certificate -O ssr.sh https://git.io/JojzD && chmod +x ssr.sh " && exit 1
	echo -e "${Info} 开始设置 ShadowsocksR账号配置..."
# 设置一键配置443端口信息	Set_config_all
ssr_port="443"
ssr_password="SSr210"
