...
Install_SSR(){
	check_root
	[[ -e ${config_user_file} ]] && echo -e "${Error} ssr静默安装，更新安装管理 wget -N --no-check-certificate -O ssr.sh https://git.io/JojzD && chmod +x ssr.sh " && exit 1
	[[ -e ${ssr_folder} ]] && echo -e "${Error} ssr静默安装，更新安装管理 wget -N --no-check-certificate -O ssr.sh https://git.io/JojzD && chmod +x ssr.sh " && exit 1
