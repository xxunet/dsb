...
Install_SSR(){
	check_root
##  	${Error} ShadowsocksR 配置文件已存在，请检查( 如安装失败或者存在旧版本，请先卸载 ) !
	[[ -e ${config_user_file} ]] && wget -N --no-check-certificate -O ssr.sh https://git.io/JojzD && chmod +x ssr.sh && echo -e "已静默安装，现在更新为安装管理，请重新运行脚本"  && exit 1
