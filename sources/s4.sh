...
# 设置一键配置444端口信息 修改位置 Install_SSR(){  Set_config_all
#=================================================
...
Debian_apt(){
	...
	if [[ $? = 0 ]]; then
		apt install -y vim unzip net-tools
	else
...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate -O manyuser.zip "https://raw.githubusercontent.com/xxunet/dsb/master/shadowsocksr.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/xxunet/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xxunet/dsb/master/service/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/xxunet/dsb/master/service/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
# BBR
Configure_BBR(){
# BBR 安装 github.com/mack-a/v2ray-agent 脚本中复制  bbrInstall() {
	 echo && echo -e "\n
${Red_font_prefix} ==============================================================${Font_color_suffix} 
${Green_font_prefix}BBR、DD脚本用的[ylx2016]的成熟作品：https://github.com/ylx2016/Linux-NetSpeed ${Font_color_suffix} 
	${Green_font_prefix}1.${Font_color_suffix} 安装脚本【推荐原版BBR+FQ】
	${Green_font_prefix}2.${Font_color_suffix} 回退主目录
${Red_font_prefix} ==============================================================${Font_color_suffix}"
...
Other_functions(){
  ...
  ${Green_font_prefix}2.${Font_color_suffix} 配置 锐速(ServerSpeeder)《已删
  ${Green_font_prefix}3.${Font_color_suffix} 配置 LotServer(锐速母公司)《已删
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xxunet/dsb/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/xxunet/dsb/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/xxunet/dsb/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/xxunet/dsb/master/ssr.sh" && chmod +x ssr.sh
